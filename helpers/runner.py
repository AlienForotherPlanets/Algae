...
class Runner:
    ...
    def runPreprocess(self, job):
        ...
        for pre in job.preprocessors:
            # print the name
            io.printIndented(
                "running preprocessor '{}'...".format(pre.name), 1)
            ...
            if self.shouldPreprocess(jobName, pre.name, pre.runThisRound):
                ...
                assignments = getJobFromConfig(
                    self.config, jobName).assignments
                args = pre.args
                helpers = self.corpus
                ...
                try:
                    ...
                    # update progress
                    self.progress.updatePreprogress(
                        job.name, pre.name, success)
    ...
    def runPostprocess(self, job):
        ...
        for post in job.postprocessors:
            # print the name
            io.printIndented(
                "running postprocessor '{}'...".format(post.name), 1)
