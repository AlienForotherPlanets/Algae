...
class Corpus:
    def __init__(self, config):
        ...
        if os.path.exists(config.corpusPath) == False:
            io.printErrorAndExit(
                "corpus not found at {}".format(config.corpusPath))
    ...
    def getPreprocessedPath(self, student, assignment, filename):
        path = self.config.corpusPath + student + \
            '/' + assignment + '/__algae__/' + filename
        if os.path.exists(path):
            return path
        else:
            return None
    ...
    def getPostprocessedPath(self, assignment, filename):
        path = self.config.corpusPath + \
            '__algae__/postprocessed/' + assignment + '/' + filename
        if os.path.exists(path):
            return path
        else:
            return None
    ...
    def writeToPreprocessed(self, text, student, assignment, filename):
        ...
        folderPath = self.config.corpusPath + student + \
            '/' + assignment + '/__algae__/' + split[0]
    ...
    def writeToProcessed(self, text, assignment, filename):
        split = splitFilename(filename)
        folderPath = self.config.corpusPath + \
            '/__algae__/processed/' + assignment + '/' + split[0]
        if os.path.exists(folderPath) == False:
            os.makedirs(folderPath)
    ...
    def writeToPostprocessed(self, text, assignment, filename):
        split = splitFilename(filename)
        folderPath = self.config.corpusPath + \
            '/__algae__/postprocessed/' + assignment + '/' + split[0]
        if os.path.exists(folderPath) == False:
            os.makedirs(folderPath)
