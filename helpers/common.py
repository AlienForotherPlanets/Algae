...
def getPartner(student, assignment, semester, helpers):
    if student != None:
        partnerText = helpers.readFromAssignment(
            student, assignment, "partners.txt")
        if partnerText != None:
            partnerText = re.sub(",", "\n", partnerText)
            partnerText = re.sub(":", "\n", partnerText)
...
class PairResults:
    def __init__(self, assignment, filename, helpers):
        self.filename = helpers.config.corpusPath + \
            '__algae__/processed/' + assignment + '/' + filename
        ...
        if len(self.members) == 2 and self.allowPartners:
            ...
            if member1.partner == None or member2.partner == None:
                ...
                if self.allowMistakes == True:
                    mistake = (member1.partner == None and member2.partner == member1.student) or (
                        member2.partner == None and member1.partner == member2.student)
                    return not mistake
