...
statTypes = ["numLines", "numWhitespace", "numComments", "avgIdentLength",
             "numFunctions", "numDefines", "numMathOps", "lenLongestLine", "numReturns"]
            ...
            if student in studentDict:
                ...
                for stat in statTypes:
                    if stats[stat].deviation != 0.0:
                        total += abs(zScore(data[stat],
                                     stats[stat].mean, stats[stat].deviation))
...
def run(students, assignments, args, helpers):
    ...
    for assign in assignments:
        t = Process(target=runAssignment, args=(
            students, assign, args, helpers))
        threads.append(t)
        t.start()
